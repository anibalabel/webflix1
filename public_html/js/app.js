(function($) {
  ...
  // Inline onclick handlers require these functions on the global scope.
  window.EpisodeFunction = EpisodeFunction;
  window.SerieFunction = SerieFunction;
  window.MovieFunction = MovieFunction;
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        for (var i = 0; i < result.Episodes.length; i++) {
          List += "<div alt='" + result.Episodes[i].imdbID + "' class='poster-search poster-search-episode'>" + result.Episodes[i].Title + " (Episode : " + result.Episodes[i].Episode + ") </span><div>" + result.Episodes[i].Released + "</div></div>";
        }
        $("#result_search").html(List);
        $("#div1").show();
  ...
  $("#search_series").on("click",function() {
    ...
    $.ajax({
      ...
      success: function(result) {
        var List = "";
        for (var i = 0; i < result.Search.length; i++) {
          List += "<div alt='" + result.Search[i].imdbID + "' class='poster-search poster-search-serie'><img src='" + result.Search[i].Poster + "'/><span>" + result.Search[i].Title + "</span><div>" + result.Search[i].Year + "</div></div>";
        }
        $("#result_search").html(List);
        $("#div1").show();
  ...
  $(document).on("click", ".poster-search-episode", function() {
    EpisodeFunction($(this).attr("alt"));
  });
  $(document).on("click", ".poster-search-serie", function() {
    SerieFunction($(this).attr("alt"));
  });
